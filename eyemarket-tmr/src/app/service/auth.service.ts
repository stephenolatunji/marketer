...
export class AuthService {
  ...
  isAuthenticated() {
    if(localStorage.getItem('shuke') !== null && localStorage.getItem('userId')!==null) {
  ...
  checkAuth(userId) {
    if(userId!==null) {
      ...
      if( 
        ...
        tokenFromCookie.slice(10,11) === loggedInUserId.slice(0,1).toLowerCase() &&
        tokenFromCookie.slice(21,22) === loggedInUserId.slice(2,3).toLowerCase()) {
          return true
        }
        else {
          return false;
